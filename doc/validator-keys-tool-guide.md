...
change if the rmcd config and/or server are compromised.
...
*   sign tokens authorizing a rmcd server to run as the validator identified
    by this public key.
...
The current token needs to be present in the rmcd config file.
...
Sample output:
```
  Validator keys stored in /home/ubuntu/.rmc/validator-keys.json
```
...
```
  Update rmcd.cfg file with these values:
...
For a new validator, add the [validator_token] value to the rmcd config file.
...
value. After the config is updated, restart rmcd.
...
```
  ...
  Update rmcd.cfg file with these values and restart rmcd:
...
restart rmcd. Rename the old key file and generate new [validator keys](#validator-keys) and
